...
GenerateMeshUV<Real>::UVComputeModel::UVComputeModel(
    ...
    std::function<void(unsigned int)> const* inProgress)
    :
    ComputeModel(inEngine, inFactory, inNumThreads),
    progress(inProgress)
{
}
...
void GenerateMeshUV<Real>::SolveSystemGPU(unsigned int numIterations)
{
    ...
    mVGBuffer = std::make_shared<StructuredBuffer>(
    ...
    mVGDBuffer = std::make_shared<StructuredBuffer>(
    ...
    mOVBuffer = std::make_shared<StructuredBuffer>(
    ...
    for (int j = 0; j < 2; ++j)
    {
        mTCoordsBuffer[j] = std::make_shared<StructuredBuffer>(mNumVertices,
            sizeof(Vector2<Real>));
    ...
    for (unsigned int i = 1; i <= numIterations; ++i)
    {
        ...
        mCModel->engine->Execute(cshader, numXGroups, numYGroups, 1);
