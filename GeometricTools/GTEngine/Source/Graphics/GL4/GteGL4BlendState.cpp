...
GL4GraphicsObject* GL4BlendState::Create(GraphicsObject const* object)
{
    if (object->GetType() == GT_BLEND_STATE)
    {
        return new GL4BlendState(static_cast<BlendState const*>(object));
    }
...
BlendState* GL4BlendState::GetBlendState()
{
    return static_cast<BlendState*>(mGTObject);
}

void GL4BlendState::Enable(HGLRC)
{
    ...
    if (mEnableIndependentBlend)
    {
        for (unsigned int i = 0; i < BlendState::NUM_TARGETS; ++i)
        {
            ...
            if (target.enable)
            {
                ...
                glColorMaski(i, target.rMask, target.gMask, target.bMask,
                    target.aMask);
    ...
    else
    {
        ...
        if (target.enable)
        {
            ...
            glColorMask(target.rMask, target.gMask, target.bMask,
                target.aMask);
