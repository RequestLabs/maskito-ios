...
{
    delete[] mBones;
    Deallocate2(mWeights);
    Deallocate2(mOffsets);
}
...
SkinController::SkinController(int numVertices, int numBones,
    ...
    mCanUpdate(false)
{
    mBones = new Node*[mNumBones];
    mWeights = Allocate2<float>(mNumBones, mNumVertices);
    mOffsets = Allocate2<Vector4<float>>(mNumBones, mNumVertices);
}
