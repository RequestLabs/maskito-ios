    ...
    std::vector<unsigned char> const& gsBytecode)
{
    ...
    {
        vshader.reset(static_cast<VertexShader*>(new Shader(hlslVShader)));
    }
    else
    {
        return nullptr;
    ...
    {
        pshader.reset(static_cast<PixelShader*>(new Shader(hlslPShader)));
    }
    else
    {
        return nullptr;
    ...
    if (gsBytecode.size() > 0)
    {
        ...
        {
            gshader.reset(
                static_cast<GeometryShader*>(new Shader(hlslGShader)));
        }
        else
        {
            return nullptr;
    ...
    std::shared_ptr<HLSLVisualProgram> program =
        std::make_shared<HLSLVisualProgram>();
    ...
    std::string const& csName, std::string const& csSource)
{
    ...
    if (hlslCShader.IsValid())
    {
        std::shared_ptr<ComputeShader> cshader(
            static_cast<ComputeShader*>(new Shader(hlslCShader)));

        std::shared_ptr<HLSLComputeProgram> program =
            std::make_shared<HLSLComputeProgram>();
