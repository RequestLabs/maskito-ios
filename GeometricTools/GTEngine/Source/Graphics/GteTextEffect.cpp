...
TextEffect::TextEffect(ProgramFactory& factory,
    std::shared_ptr<Texture2> const& texture)
{
    ...
    {
        mTranslate =
            std::make_shared<ConstantBuffer>(sizeof(Vector2<float>), true);
        mColor =
            std::make_shared<ConstantBuffer>(sizeof(Vector4<float>), true);
